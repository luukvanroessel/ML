"""Find the images most similar to a query image.
    ...
    uv run python -m image_search.search path/to/query.jpg -k 5
"""
...
def rank(index: Index, query: np.ndarray, k: int) -> list[tuple[str, float]]:
    ...
    The whole search is one line of maths. Because every stored vector and
    the query are unit length, the dot product between them equals the
    ...
    512x1 multiply -- still only ~20M multiply-adds, a few milliseconds.
    Brute force stays honest for a surprisingly long time; a vector
    database (v2) earns its keep in the millions, or when you want the
    index to live somewhere other than this process's memory.
    """
    sims = index.vectors @ query

    # argsort ascending, take the tail, reverse it -> k highest, best first.
...
def main() -> None:
    ...
    if index is None:
        raise SystemExit(f"no index in {args.index_dir} -- run image_search.index first")
    if not args.query.is_file():
        raise SystemExit(f"no such file: {args.query}")

    # The query must go through the exact same model as the library, or the
    # two vectors are not in the same space and the numbers mean nothing.
    backend = get_backend(index.backend)
    query = backend.embed_paths([args.query])[0]

    print(f"\nquery: {args.query}")
    print(f"searching {len(index)} images ({index.backend})\n")
    for position, (path, score) in enumerate(rank(index, query, args.k), start=1):
