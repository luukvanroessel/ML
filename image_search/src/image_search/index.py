...
Run it again after adding photos: only the new files get embedded.
"""
...
def find_images(root: Path) -> list[Path]:
    ...
    return sorted(
        p for p in root.rglob("*") if p.suffix.lower() in IMAGE_SUFFIXES and p.is_file()
    )
...
def build(
    ...
    # Reuse work from the previous run, but only if it came from the same
    # model. Vectors from two different models live in different spaces and
    # comparing them is meaningless, so a backend change means a full rebuild.
    old = load(index_dir)
    ...
    if old is not None and old.backend == backend_name:
        ...
        print(f"backend changed ({old.backend} -> {backend_name}), rebuilding")
    ...
    if todo:
        ...
        print(f"loaded {backend.name} (dim={backend.dim}) on {backend.device}")
        start = time.perf_counter()
        fresh = backend.embed_paths(todo, batch_size=batch_size)
        elapsed = time.perf_counter() - start
        print(f"embedded {len(todo)} images in {elapsed:.1f}s "
              f"({len(todo) / elapsed:.1f} img/s)")
        for file, vector in zip(todo, fresh):
            reusable[hashes[file]] = vector

    # Rebuild in `files` order. Deleted images simply drop out, because we
    # only ever look up hashes that are still on disk.
    vectors = np.stack([reusable[hashes[f]] for f in files]).astype(np.float32)
    index = Index(
    ...
    print(f"saved index: {len(index)} vectors of dim {vectors.shape[1]} -> {index_dir}")
    return index


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("library", type=Path, help="folder of images to index")
    ...
    build(args.library, args.index_dir, args.backend, args.batch_size)
